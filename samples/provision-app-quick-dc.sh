...
RESPONSE=$(curl -s -X POST $BROKER_URL/v1/intention/open?ttl=30\&quickstart=true \
    ...
    -d @<(cat provision-app-quick-dc.json | \
...
echo -n $INTENTION_ID > provision-app-quick-dc.intention.id
# echo "Hashed transaction.id: $(echo -n $INTENTION_TOKEN | shasum -a 256)"

echo "===> DC Build"

# Not shown: Build DC and create artifact
echo "===> ..."
echo "===> DC Build - Success!"
# Patch action with build info
ACTIONS_BUILD_TOKEN=$(echo $RESPONSE | jq -r '.actions.dcbuild.token')
SOURCE_URL="ghcr.io/bcgov/nr-nodejs-sample/nodejs-sample:d$PACKAGE_VERSION"
PATCH_JSON_STR='{"package":{"checksum": "sha256:'$sha256'", "size": '$RANDOM', "source": "'"$SOURCE_URL"'", "type": "oci-archive" }}'
echo "===> Patch"
...
echo -n $sha256 > provision-app-quick-dc.artifact.sha256
echo "sha256: $sha256"
